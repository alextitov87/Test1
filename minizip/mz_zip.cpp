...
int32_t mz_zip_entry_read(void *handle, void *buf, int32_t len) {
    ...
    if (read > 0)
        zip->entry_crc32 = static_cast<uint32_t>(mz_crypt_crc32_update(zip->entry_crc32, static_cast<uint8_t*>(buf), read));
...
int32_t mz_zip_entry_write(void *handle, const void *buf, int32_t len) {
    ...
    const void* test=buf;
    const uint8_t* buf_uint8=static_cast<const uint8_t*>(buf);
#ifndef MZ_ZIP_NO_CRYPTO
    if (written > 0)
        zip->entry_crc32 = mz_crypt_crc32_update(zip->entry_crc32, static_cast<const uint8_t*>(buf), written);
